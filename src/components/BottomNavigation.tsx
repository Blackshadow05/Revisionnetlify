...
export default function BottomNavigation() {
  ...
  const navItems = [
    ...
    { href: '/menus', label: 'Menú', icon: (
      <img src="/icons_custom/food.png" alt="Menú" className="w-6 h-6" />
    )},
  ];
    ...
    <>
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] bg-white border-t border-gray-200 shadow-lg">
        <nav className="flex items-center justify-around py-2 px-4">
          {navItems.map((item) => (
            <Link
              ...
              className={`flex flex-col items-center justify-center gap-1 p-2 transition-colors ${
                isActive(item.href) ? 'text-[#c9a45c]' : 'text-gray-600 hover:text-[#c9a45c]'
              }`}
            >
              {item.icon}
          ...
          <Link
            href="/configuracion"
            className="flex flex-col items-center justify-center gap-1 p-2 text-gray-600 hover:text-[#c9a45c] transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
