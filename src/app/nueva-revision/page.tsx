...
const requiredFields: (keyof RevisionData)[] = [
  ...
  'sombrero', 'bolso_yute', 'camas_ordenadas', 'room_move'
];
...
export default function NuevaRevision() {
  ...
  // 🚀 OPTIMIZACIÓN 1: Memoizar cálculo pesado del siguiente campo vacío
   const nextEmptyField = useMemo(() => {
    return requiredFields.find(f => {
      if (f === 'room_move' && formData.caja_fuerte !== 'Room Move') return false;
      return !formData[f];
    }) || null;
  }, [formData]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const emptyFields = requiredFields.filter(field => {
        if (field === 'room_move' && formData.caja_fuerte !== 'Room Move') return false;
      ...
      const finalData = {
        ...
        room_move: submitData.room_move || '',
      ...
      const shouldShare = ['Check in', 'Upsell', 'Back to Back', 'Room Move'].includes(finalData.caja_fuerte);
  ...
  const showEvidenceFields = ['Check in', 'Upsell', 'Back to Back', 'Check out', 'Guardar Upsell', 'Room Move'].includes(formData.caja_fuerte);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <form onSubmit={handleSubmit} className="rounded-xl shadow-xl p-4 md:p-8 border border-[#3d4659] bg-white md:bg-[#334d50] gradient-form">
          ...
          <div className="space-y-6">
            ...
            <fieldset>
              ...
              <div ref={fieldRefs.current['caja_fuerte']}>
                <ButtonGroup
                  ...
                  options={['Si', 'No', 'Check in', 'Check out', 'Upsell', 'Guardar Upsell', 'Back to Back', 'Show Room', 'Room Move']}
            ...
            {/* Room Move Field */}
            {formData.caja_fuerte === 'Room Move' && (
              <fieldset>
                <legend className="sr-only">Movimiento de casita</legend>
                <FormField
                  label="¿Movimiento de casita?"
                  required={formData.caja_fuerte === 'Room Move'}
                  highlight={highlightedField === 'room_move'}
                  icon={
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
